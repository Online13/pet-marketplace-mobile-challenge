import { ActivityData, CategoryData, MarketplaceData, MessageData, PetData } from "./type";
...
export const ACTIVITY_DATA: ActivityData[] = [
	...
	},
	{
		...
		description: "Lorem ipsum dolor sit amet consectetur adipisicing elit. Voluptas optio odio ullam accusamus doloribus hic eius, tenetur culpa nesciunt cum nostrum? Voluptates, architecto consequatur saepe aspernatur debitis adipisci ipsum atque.",
...
function generatorText() {
	const ref = "Lorem ipsum dolor, sit amet consectetur adipisicing elit. Accusantium, suscipit corporis recusandae ducimus vero cum laborum, nihil laudantium culpa amet itaque iste. Magni molestias est deleniti doloremque necessitatibus numquam nam?";
	const words = ref.split(' ');
	for (let i = 0; i < words.length; i++) {
		const [x, y] = [Math.floor(Math.random() * words.length), Math.floor(Math.random() * words.length)]
		let temp = words[x];
		words[x] = words[y];
		words[y] = temp;
	}
...
export const MESSAGE_DATA: MessageData[] = Array.from({ length: 30 }).map((_, i) => {
	return {
		id: i,
